...
describe("deltaForSeries", () => {
  ...
  it("sorts unsorted input by date before picking latest/prior/baseline", () => {
    const d = deltaForSeries([
      r("Ascending aorta diameter", "2026-01-01", 4.0, "cm"),
      r("Ascending aorta diameter", "2018-01-01", 3.2, "cm"),
      r("Ascending aorta diameter", "2021-01-01", 3.6, "cm"),
    ])!;
    expect(d.latest.value).toBe(4.0);
    expect(d.prior.value).toBe(3.6); // immediately before latest
    expect(d.baseline!.value).toBe(3.2); // first overall
    expect(d.vsPrior.abs).toBeCloseTo(0.4);
    expect(d.vsBaseline!.abs).toBeCloseTo(0.8);
  ...
  it("returns one entry per marker with ≥2 readings, sorted by name, skipping singletons", () => {
    const client = {
      results: [
        r("Aortic valve mean gradient", "2021-01-01", 10),
        r("Aortic valve mean gradient", "2026-01-01", 14),
        r("Ascending aorta diameter", "2021-01-01", 3.2, "cm"),
        r("Ascending aorta diameter", "2026-01-01", 4.0, "cm"),
