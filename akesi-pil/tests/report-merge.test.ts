...
describe("applyReportContribution — diseases", () => {
  it("adopts a provenance-less match instead of duplicating", () => {
    const c = base();
    c.factors!.diseases = [{ id: "d1", date: "2021-10-15", diagnostic: "CAC: 34; CAD-RADS 1 in the Mid-LAD" }];
    const res = applyReportContribution(c, "src1", [
      { date: "2021-10-15", diagnostic: "cac: 34; cad-rads 1 in the mid-LAD." },
    ], []);
  ...
  it("is idempotent: re-applying keeps counts flat", () => {
    ...
    const mk = [mr("CAC score", 34, "2021-10-15", "src1")];
...
describe("applyReportContribution — markers", () => {
  it("adopts a provenance-less Imaging reading by marker|date", () => {
    const c = base();
    c.results = [mr("CAC score", 34, "2021-10-15")];
    const res = applyReportContribution(c, "src1", [], [mr("CAC score", 34, "2021-10-15", "src1")]);
...
describe("applySourceReadings (positional adoption)", () => {
  it("adopts a provenance-less match, adds new, leaves already-sourced alone", () => {
    const c = base();
    c.results = [
      blood("ApoB", 76, "2026-05-19"),            // provenance-less → adopt
      blood("LDL-C", 109, "2026-03-18", "srcOld"), // already sourced → leave
    ];
    const res = applySourceReadings(c, "src8", [
      blood("ApoB", 76, "2026-05-19"),
      blood("LDL-C", 109, "2026-03-18"),
      blood("Lp(a)", 30, "2026-05-19"),           // new → add
    ]);
  ...
  it("lets a real reading oust a fromComparison placeholder", () => {
    const c = base();
    c.results = [{ ...blood("Lp(a)", 30, "2026-05-19"), fromComparison: true }];
    const res = applySourceReadings(c, "src8", [blood("Lp(a)", 30, "2026-05-19")]);
  ...
  it("is idempotent on re-run", () => {
    const c = base();
    const rows = [blood("ApoB", 76, "2026-05-19")];
  ...
  it("leaves an existing value untouched without refresh", () => {
    const c = base();
    c.results = [blood("ANA titer", 1, "2026-05-19", "src8")];
    const incoming: MarkerResult = { ...blood("ANA titer", 80, "2026-05-19"), unit: "titer", valueText: "1:40 to 1:80" };
